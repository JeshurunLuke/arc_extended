...
from arc_extended._database import UsedModulesARC
from arc_extended.divalent_atom_functions import DivalentAtom
from arc_extended.wigner import Wigner6j, CG
...
class RET: 
    ...
    def find_energy(self, n, l, Eref = 0,  debug = False):
        ...
        eg = self.StarkScan.y
        indOIc = []
        res = np.zeros((len(statesOI), len(eigVecs)))
        
        for state in statesOI: 
            stateInit = np.zeros(len(basisStates))
            stateInit[state] = 1.0

            indOIc.append(np.argmax(np.abs(np.array(eigVecs[-1]).conj().T @ stateInit)))
        startPoint = min(np.array(indOIc))
        
        for state in range(len(statesOI)): 
    ...
    def find_resonance(self,n,l, debug = False): 
        ...
        for state in range(len(statesOI)): 
            ...
            for scan_param in range(len(self.StarkScan.y)):
                ...
                stateInit = eigVecs[scan_param][:, indOI]
...
class StarkMap:
    ...
    def defineBasis(
        ...
        else: # Added for off-diagonal electric field
            for tn in xrange(nMin, nMax):
                for tl in xrange(min(maxL + 1, tn)):
                    for tj in np.linspace(tl - s, tl + s, round(2 * s + 1)):
    ...
    def diagonalise(
        ...
        if isinstance(theta, (int,  float, complex, np.float64)): 
            theta = [theta] 
        if isinstance(phi, (int,  float, complex, np.float64)): 
            phi = [phi]
        if isinstance(Earray, (int,  float, complex, np.float64)):
            Earray = [Earray]
        if isinstance(Barray, (int,  float, complex, np.float64)): 
            Barray = [Barray]
    ...
    def displayLevels(self, units='cm', progressOutput=False, debugOutput=False, displayAll=False):
        ...
        for br in xrange(len(self.y)): # br is the length of the eFieldList
            y = np.array(self.y[br]*self.scaleFactor)
            yState = np.array(self.highlight[br])
            yComp = np.array(self.composition[br],  dtype="object")
            #print(len(yState))
            
            filterY = (y > lowerY) & (y < upperY)
            y = y[filterY]
            yState = yState[filterY]
            yComp = yComp[filterY]
            #print(len(yState))
            
            if displayAll:
                # print(yState)                                    
                yState = np.array([1 for jj in yState])
                #print(len(yState))
            
            filterYstate = yState > 1e-4
            y = y[filterYstate]
            ...
            for i in xrange(len(y)):
                stateNames.append(self._stateComposition3(yComp[i]))
                #print(self._stateComposition(yComp[i]) +
                        #("\n Energy: %.2f" %(y[i]*self.scaleFactor)))
            #'E field [V/cm]':eFieldList, 
            d = {'Energy [GHz]': y, 'dE [MHz]': (y-min(y))*1e3, 'State': stateNames, 'Overlap': yState}            
    ...
    def _stateComposition2(self, stateVector, upTo=300,totalContributionMax = 0.999):
        ...
        if upTo == -1:
            ...
            while (index > -upTo) and (totalContribution < totalContributionMax):
                ...
                index -= 1
        return mainStates
    def _stateComposition3(self, stateVector):
        i = 0
        totalContribution = 0
        value = ""        
        while (i < len(stateVector)) and (totalContribution < 0.95):
            ...
            value = value + ("%.2f" % stateVector[i][0]) +\
                "%s" % self.basisStates[stateVector[i][1].astype(int)]
            totalContribution += abs(stateVector[i][0])**2
            i += 1
